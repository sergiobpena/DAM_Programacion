...
public class ProcesadorClientesTest {
    ...
    ProcesadorClientes p = new ProcesadorClientes();
    String nif1="123X";
    ...
    @Test
    public void testNif() {
        assertTrue(p.nif(ProcesadorClientes.separaCadea(cadea2)[0])& p.nif(nif2));
    }
    @Test
    public void testNifMal(){
        assertFalse(p.nif(ProcesadorClientes.separaCadea(cadea1)[0])|p.nif(nif1)| p.nif(nif3));
    }
    @Test
    public void testNieBen(){
            assertTrue(p.nie(ProcesadorClientes.separaCadea(cadea1)[0]));
    }
    @Test
    public void testNieMal(){
        assertFalse(p.nie(ProcesadorClientes.separaCadea(cadea2)[0]));
    }
}
